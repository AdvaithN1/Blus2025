...
async function getUserInfo(userId) {
  try {
    ...
    await client.connect(mongolink, "advaithnarayanan20", pass);
...
const generateRTEToken = async (req, resp) => {
  ...
  if (req.params.role === 'publisher') {
    role = RtcRole.PUBLISHER;
    rtmRole = RtmRole.PUBLISHER;
  } else if (req.params.role === 'audience') {
    role = RtcRole.SUBSCRIBER
    rtmRole = RtmRole.Rtm_User;
  } else {
    return resp.status(400).json({ 'error': 'role is incorrect' });
  }
  // get the expire time
  ...
  const rtmToken = RtmTokenBuilder.buildToken(APP_ID, APP_CERTIFICATE, uid, RtmRole.PUBLISHER, privilegeExpireTime); //RtmRole.PUBLISHER
  // return the token
